...
/** Abstract class for adding visualizations to models. Visualize must be overloaded, and is then
 * called in the models local coord system */
class Visualizer {
private:
...
public:
  ...
  /** remove user supplied visualization to a model - supply the same ptr passed to
   * AddCustomVisualizer */
...
public:
  ...
  meters_t range; ///< Maximum distance at which blobs are detected (a little artificial, but
...
public:
  ...
  struct config_t {
    ...
    double
        close_limit; ///< How far the gripper can close. If < 1.0, the gripper has its mouth full.
...
public:
  ...
  public:
    ...
    Pose pose; ///< Absolute accurate position of the target in world coordinates (it's cheating to
    /// use this in robot controllers!)
    Model *mod; ///< Pointer to the model (real fiducial detectors can't do this!)
    int id; ///< the fiducial identifier of the target (i.e. its fiducial_return value), or -1 if
...
public:
  ...
  bool ignore_zloc; ///< Are we ignoring the Z-loc of the fiducials we detect compared to the
  /// fiducial detector?
